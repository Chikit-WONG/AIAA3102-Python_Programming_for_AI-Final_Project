"""
任务1基线推理脚本（0-shot）：客户需求分类
使用原始模型（未经微调）进行0-shot分类
"""
...
# 输出配置
output_file = './output/task1_baseline_0shot_predictions.jsonl'
save_batch_size = 10

print("="*80)
print("任务1基线推理（0-shot）：客户需求分类")
print("="*80)
print(f"模型路径: {model_id_or_path}")
print(f"方法: 0-shot (无示例)")
...
# ==================== 初始化模型 ====================
print(f"\n初始化模型（原始模型，无微调）...")
engine = PtEngine(model_id_or_path)  # 不加载adapters
...
def predict_category(engine, instruction):
    ...
    """
    # 构造输入，明确列出11个类别
    query = f"""Please classify the following user request into one of these categories:
